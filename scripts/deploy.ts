...
await hre.run("compile");
  ...
  const expectedRent = ethers.parseEther("1400"); // 1400 ETH
  const renter = (await ethers.getSigners())[0].address;
  const landlord = (await ethers.getSigners())[1].address;
  const startDate = Math.floor(new Date("2025-07-10").getTime() / 1000);
  const endDate = Math.floor(new Date("2027-07-10").getTime() / 1000);

  const rentalContract = await Rental.deploy(payDate, expectedRent, renter, landlord, startDate, endDate);
  ...
  console.log("📄 ABI written to rental-frontend/src/lib/Rental.json");
  ...
  console.log("🔑 Address written to rental-frontend/.env.local");
